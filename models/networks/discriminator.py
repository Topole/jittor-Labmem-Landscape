...
class SesameNLayerDiscriminator(BaseNetwork):
    ...
    def __init__(self, opt, input_nc=None):
        ...
        for input_nc in sizes: 
            ...
            sequence = [[nn.Conv2d(input_nc, nf, kernel_size=kw, stride=2, padding=padw),   # 64
                         nn.LeakyReLU(0.2)]]

            for n in range(1, opt.n_layers_D):  # 3层：128、256、512
                nf_prev = nf
                nf = min(nf * 2, 512)
                stride = 1 if n == opt.n_layers_D - 1 else 2 # 512那个卷积层的stride=1
    ...
    def execute(self, input):
        img, sem = input[:,-3:], input[:,:-3] # input后三位是RGB图像、前面是语义图
