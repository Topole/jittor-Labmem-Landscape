...
class SPADEGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.sw, self.sh = self.compute_latent_vector_size(opt) # 16 12
    ...
    def compute_latent_vector_size(self, opt):
        if opt.num_upsampling_layers == 'normal':  # normal
            num_up_layers = 5
        ...
        sw = opt.crop_size // (2**num_up_layers)    # 512//(2^5)=16
        sh = round(sw / opt.aspect_ratio) #12
    ...
    def execute(self, input, z=None):
        ...
        if self.opt.use_vae:
            ...
            x = self.fc(z)
            x = x.view(-1, 16 * self.opt.ngf, self.sh, self.sw) # 1*(16*ngf)*sh*sw
        else:
